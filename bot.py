...
@app.route('/')
def home():
    return "Kunwar DMS Core Server Engine is Active!", 200

# ================== CONFIGURATION ==================
TOKEN = '8644302388:AAEBx4UKSE_e7yjS5j14DHxyXeXS_HJuJUw'
URL = f'https://api.telegram.org/bot{TOKEN}/'

# 🔴 APNI ASLI TELEGRAM ACCOUNT ID YAHAN DALO
ADMIN_ID = 6752542323  
...
def init_db():
    ...
    for key, value in default_settings.items():
        set_setting_if_empty(key, value)

def set_setting_if_empty(key, value):
    conn = sqlite3.connect('bot_data.db')
    cursor = conn.cursor()
...
async def init_telethon_login(chat_id, phone):
    session_name = f"session_{chat_id}_{int(time.time())}"
    client = TelegramClient(session_name, API_ID, API_HASH)
    await client.connect()
    try:
        send_code = await client.send_code_request(phone)
        active_clients[chat_id] = {
            'client': client, 'phone': phone, 'phone_code_hash': send_code.phone_code_hash, 'session_name': session_name
        }
        user_states[chat_id] = 'expecting_otp'
        send_tg_msg(chat_id, "📩 **OTP Sent!** Please check your Telegram account and send the OTP here.")
            ...
            if chat_id in user_states:
                ...
                if state == 'expecting_phone':
                    if not text.startswith("+"):
                        ...
                        return
                    asyncio.run(init_telethon_login(chat_id, text))
                    return
                elif state == 'expecting_otp':
                    asyncio.run(verify_telethon_otp(chat_id, text.strip()))
                    return
                elif state == 'expecting_utr':
                    plan = payment_tracking.get(chat_id, "UNKNOWN")
                    user_states.pop(chat_id, None)
                    
                    # Notify Admin with UTR
                    admin_alert = f"🔔 **NEW VERIFICATION REQUEST!**\n\n👤 User ID: `{chat_id}`\n📦 Plan: {plan.upper()}\n🔢 UTR ID: `{text.strip()}`\n\n⚠️ *Check screenshot in your chat and type command below to approve:*\n`/approve {chat_id} 30`"
                    send_tg_msg(ADMIN_ID, admin_alert)
                    
                    send_tg_msg(chat_id, "⏳ **Details Received!** Now please send the **Payment Screenshot** here. Admin will verify your UTR and Screenshot to approve instantly.")
                    return
